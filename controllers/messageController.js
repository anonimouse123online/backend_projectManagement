    ...
    const result = await pool.query(
      ...
      SELECT
        ...
        (
          ...
          FROM conversation_members other_membership
          JOIN users other_user ON other_user.id = other_membership.user_id
          WHERE other_membership.conversation_id = c.id
...
exports.createConversation = async (req, res) => {
  ...
  try {
    ...
    const { receiverId, receiverIds, isGroup, name } = req.body;
    ...
    if (isGroup) {
      ...
      if (!receiverIds || !Array.isArray(receiverIds) || receiverIds.length === 0) {
        throw new Error('receiverIds is required for group chats');
      }
      if (!name || !name.trim()) {
      ...
      const conversation = await client.query(
        `INSERT INTO conversations (is_group, name) VALUES (TRUE, $1) RETURNING *`,
        [name.trim()]
      );

      const conversationId = conversation.rows[0].id;
      
      // Ensure the sender is in the group
      const allMembers = Array.from(new Set([senderId, ...receiverIds]));
